...
function generateDocuments(caseId: any, checklist: Record<string, boolean>) {
    const docs: any[] = [];
    const now = new Date();
    ...
    const docTemplates: { category: DocumentCategory; title: string; checklistKey: keyof typeof checklist }[] = [
        ...
        { category: DocumentCategory.INCIDENT_PHOTO, title: 'Incident Scene Photos', checklistKey: 'incidentPhotos' },
        { category: DocumentCategory.WITNESS_STATEMENT, title: 'Witness Statement', checklistKey: 'witnessStatements' },
        { category: DocumentCategory.PAY_STUB, title: 'Pay Stubs - 6 Months', checklistKey: 'payStubs' },
        { category: DocumentCategory.EMPLOYER_LETTER, title: 'Employer Lost Wage Letter', checklistKey: 'employerLetter' },
        { category: DocumentCategory.INSURANCE_POLICY, title: 'Insurance Policy Declaration', checklistKey: 'insuranceDocs' },
    ];
        ...
        if (checklist[template.checklistKey]) {
            docs.push({
                ...
                file: {
                    ...
                    hash: `hash_${require('crypto').randomUUID()}`,
                },
                aiAnalysis: {
                    isProcessed: true,
                    processedAt: now,
...
async function seed() {
    ...
    console.log(`Debug statement: MONGODB_URI is ${process.env.MONGODB_URI ? 'SET' : 'NOT SET'}`);

    // Dynamic imports to ensure env vars are loaded first
    const mongoose = (await import('mongoose')).default;
    const connectToDatabase = (await import('../lib/db/connect')).default;
    const Case = (await import('../lib/db/models/Case')).default;
    const Document = (await import('../lib/db/models/Document')).default;
    const Lien = (await import('../lib/db/models/Lien')).default;
    ...
    for (const testCase of TEST_CASES) {
        ...
        const newCase = await Case.create({
            ...
            client: {
                firstName: testCase.client.name.split(' ')[0],
                lastName: testCase.client.name.split(' ').slice(1).join(' '),
                email: testCase.client.email,
                phone: testCase.client.phone,
            },
            incidentDescription: testCase.incident.description,
            incidentLocation: `${testCase.incident.location.address}, ${testCase.incident.location.city}, ${testCase.incident.location.state}`,
            incidentDate: testCase.incident.date,
            defendant: 'Defendant TBD',
            injuries: ["Whiplash", "Back Pain"], // Default added as it's missing in seed data but present in schema
            insurance: {
                defendantPolicy: {
            ...
            // evidenceChecklist is not in Case schema, handled via Document existence or separate logic? 
            // Case schema has documentCount and pendingActionCount.
            // But let's check Case.ts again. It has evidenceChecklist? NO.
            // It has aiMetadata.
        ...
        // Create documents
        const docs = generateDocuments(newCase._id, testCase.evidenceChecklist);
        if (docs.length > 0) {
        ...
        for (const lien of testCase.liens) {
            await Lien.create({
                ...
                originalAmount: lien.amount,
                currentAmount: lien.amount,
                amountPaid: 0,
