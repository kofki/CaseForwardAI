...
import { ActionStatus, ActionPriority, InputSourceType, AuditEventType, Severity } from '../../db/types/enums';

// Map ActionCard type to Action type
const ACTION_TYPE_MAP: Record<string, string> = {
    'DRAFT_EMAIL': 'send_client_email',
    'DRAFT_SMS': 'send_client_text',
    'SCHEDULE_EVENT': 'general_recommendation',
    'FLAG_CONTRADICTION': 'flag_document_issue',
    'REQUEST_DOCUMENTS': 'request_missing_document',
...
export async function runRoundTableAndPersist(
    ...
    const action = await Action.create({
        ...
        actionType: ACTION_TYPE_MAP[card.type] || 'general_recommendation',
        ...
        aiAnalysis: {
            ...
            clientCommunication: {
                draftType: card.type === 'DRAFT_EMAIL' ? 'email' : card.type === 'DRAFT_SMS' ? 'text' : 'email',
